...
def register():
    ...
    if form.validate_on_submit():
        ...
        if User.query.filter_by(email=form.email.data).first():
            flash("You have already signed up with this email! Routed you to Login Page instead.", "error")
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            flash("You must be logged in to comment. Redirected you to Login Page.", "error")
            return redirect(url_for('login'))
        new_comment = Comment(
